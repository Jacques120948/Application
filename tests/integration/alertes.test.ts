...
beforeEach(async () => {
  ...
  await prisma.plan.update({ where: { id: 'builder' }, data: { alertsPerMonth: 3 } })
})

afterAll(async () => {
  await prisma.plan.update({ where: { id: 'builder' }, data: { alertsPerMonth: 0 } })
...
describe('ce qui arrête les envois', () => {
  ...
  it('arrête une application qui s’emballe, même si le quota du mois reste ouvert', async () => {
    await prisma.plan.update({ where: { id: 'builder' }, data: { alertsPerMonth: 100_000 } })
