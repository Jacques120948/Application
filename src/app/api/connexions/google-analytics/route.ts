...
import { estConfigure, urlAutorisation } from '@/server/integrations/providers/google-analytics'
import { assertSameOrigin, fail, ok } from '@/server/http/respond'

/**
 * Commence l'autorisation Google Analytics : renvoie l'adresse où envoyer la personne.
 *
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await assertConnectionSlot(user.id, 'google-analytics')
    return ok({ url: urlAutorisation(creerEtat(user.id, 'google-analytics')) })
  } catch (error) {
    return fail(error)
  }
}
